...
R에서 패키지를 설치하는 방법은 크게 두가지이다. 콘솔창에 install.packages("패키지 이름")를 사용하거나, RStudio menu -> Tools -> Install Packages에서 패키지 이름을 찾거나 입력해서 설치하는 방법이다. 통상적으로는 콘솔창에 install.packages("") 명령어를 활용한다. 
...
* readr::read_csv()는 쉼표로 값이 구분된 텍스트파일(.csv)을 불러올때 활용한다. 
* readxl::read_excel()은 엑셀파일(.xlsx)을 불러올때 활용한다. 
* haven::read_sav()는 SPSS로 저장된 파일(.sav)을 불러올때 활용한다. 
* read.delim()은 탭으로 구분된 텍스트파일을 불러올때 활용한다. 
* 데이터를 불러올때는 파일의 구분자, 인코딩, 변수형 등을 잘 확인해야 한다. 
* SPSS 파일의 변수 라벨과 값 라벨은 haven::read_sav()으로 불러오면 속성 정보로 함께 유지된다. 필요에 따라 활용하라

아래의 코드를 좀더 자세히 살펴보자. 첫번째 코드는 나의 프로젝트 또는 작업 디렉토리 안에 있는 data_csv.csv파일을 불러와서 data_csv라는 데이터 프레임 객체를 생성하라는 뜻이다. 마찬가지로 두번째 코드는 spss 파일을 불러오는 코드이다. 
csv, xlsx, sav 등 주요 파일 형식은 R에서 직접 import할 수 있다. 특히 SPSS의 변수 라벨이나 값 라벨 정보를 유지해야 한다면 sav 파일을 haven::read_sav()으로 직접 불러오는 것이 좋다. 


```{R, eval=FALSE}
data_csv <- readr::read_csv("data_csv.csv")
data_excel <- readxl::read_excel("data_excel.xlsx")
data_spss <- haven::read_sav("data_sav.sav")
```
## dplyr 패키지의 이해
...
* R보다 복잡한 자료변형이 가능하다. 
* pipe operator(%>%)를 사용하여 코드를 심플하게 짤 수 있다. 
...
먼저 pipe operator에 대해 이해하면 dplyr의 강점을 십분 이해할 수 있다. 남이 짜둔 코드를 보다보면 마치 에러메시지와 같은 형태의 %>% 가 자주 등장한다. %>% 은 pipe operator라 불리며 "and then"의 문법과 같은 의미로 사용된다. 원래 R의 기본 코드는 괄호()가 이 기능을 수행한다. 
...
코드 시현에 앞서서 R에 내장된 데이터를 불러와보자. R에는 연구자가 코드를 연습해볼 수 있는 다양한 종류의 데이터가 많이 있다. 여기서는 `nycflights13` 패키지가 제공하는 `flights` 데이터를 사용한다. 이 데이터에는 2013년 뉴욕의 세 공항에서 출발한 항공편 정보가 담겨 있다.
...
설치된 패키지를 불러온 다음 `flights`를 일반 데이터 프레임으로 변환한다.

```{r, collapse=TRUE, message=FALSE, warning=FALSE}
library(nycflights13)
flight_df <- data.frame(nycflights13::flights)
print(head(nycflights13::flights), width = 70) # 처음 6개 행을 간결하게 출력
str(flight_df)
```

nycflights13을 실행시키면 flights라는 데이터를 사용할 수 있게 된다. 가장 먼저 데이터의 구조를 판단하고, dataframe으로 변환하는 작업이 필요하다. 

* `head()`는 자료의 처음 6개 행을 보여준다. 출력 너비에 따라 화면에 표시되지 않은 열은 아래에 별도로 안내된다. 출력 결과를 살펴보면 `year`, `month`, `day`, `dep_time` 등의 변수가 정수형 자료로 제시되어 있다.
...
* starts_with, ends_with, contains를 활용하면 변수명을 조합하여 선택도 가능하다. 
...
* mutate의 결과를 이후에도 사용하고 싶다면 새로운 데이터 프레임 또는 기존 데이터 프레임에 저장하는 명령어(->newdataframename)를 추가한다. 
...
#연산하여 새로운 변수를 생성
flight_df %>%
  mutate(speed=distance/air_time*60) -> flight_df_mutate1
...
#기존 변수를 제외하고 새로운 변수만 남기는 경우
flight_df %>% 
  mutate(total_min=hour*60+minute, .keep="none") -> flight_df_mutate2
```
...
```{r group-by-summarise-diagram, echo=FALSE, fig.cap='성별로 자료를 묶고 집단별 평균을 계산하는 과정', fig.align='center', out.width='100%'}
knitr::include_graphics("images/group-by-summarise.png")
```
...
* n_distinct(x) : 중복없는 유일한 관측치 개수 계산
...
결과값을 살펴보면 no delay 집단의 경우 arr_delay 값이 음수값을 가지기 때문에 통계값 역시 음수값을 갖는 것으로 확인되었다. 문제는 na 집단이다. 에러메시지가 뜨는 것을 확인할 수 있다. na 집단을 제외하고 다시 summarise를 해보도록 하자. 앞에서 쓴 코드에 filter 명령어만 삽입하면 간단하게 해결된다. !is.na()가 na가 아닌 row만 선택하는 옵션임을 기억하자. 마지막으로 summarise 함수의 작동방식을 제대로 이해하기 위해 아웃풋을 final이라는 dataframe으로 별도로 저장해보자. 또는 특정한 통계량만 na를 제외하고 분석하고 싶다면 na.rm=TRUE 옵션을 삽입하면 된다(예: per25_s = quantile(arr_delay, 0.25, na.rm = TRUE))
...
right_join 결과를 살펴보면 data2의 행은 모두 살아있다. data1은 세명의 데이터만 있기 때문에 매칭되지 않는 행에서는 data1에서 가져오는 변수의 값이 모두 na로 표시되어 있는 걸 확인할 수 있다. 

#### inner_join :첫번째 데이터와 두번째 데이터의 교집합 행만 결합

inner_join은 첫번째 데이터와 두 번째 데이터에 공통으로 존재하는 key 값에 해당하는 행만 호출한다. inner_join을 보여주기 위해 data1에 하나의 열을 추가해보도록 하겠다. 

```{r, collapse=TRUE}
army <-data.frame(id=8, name="army", solo=NA)
data1 <- bind_rows(data1, army)
data1
data1 %>% 
  inner_join(data2, by="id")
```

data1에는 id 1,2,3,8이 포함되어 있고, data2에는 id 1,2,3,4,5,6,7이 포함되어 있다. 따라서 inner_join의 결과는 두 데이터의 교집합인 1,2,3만 포함되는 것을 확인할 수 있다. 
...
지금까지 살펴본 데이터들은 이미 정련이 되어 있는 tidy data이다. tidy data란 무엇인가? tidyverse를 만든 해들리 위컴^[Wickham, H. & Grolemund, G. (2017). R for Data Science]의 정의에 따르면 tidy data는 다음과 같은 세개의 조건을 만족해야 한다. 
...
* cols : 현재는 변수로 지정되어 있지만 값(value)으로 변환할 column 이름 (지정방식은 select() 함수와 동일). 이 데이터에서는 year2016~year2019이다. 
* names_to : 값으로 변환될 자료들의 변수 이름. 이 데이터에서는 year이다. 
* values_to : wide하게 분산되어 있는 값들의 변수 이름.이 데이터에서는 sales이다. 

```{r, collapse=TRUE}
btsAlbumSales_wide %>% 
  tidyr::pivot_longer(cols=year2016:year2019, names_to="year", values_to="sales") ->long
long
```
...
```{r, collapse=TRUE}
btsAlbumSales_wide %>% 
  pivot_longer(cols=year2016:year2019, names_to="year", values_to="sales") %>% 
  mutate(year=str_replace(year, "year", ""))->long_replace
long_replace
```
### 넓게 만들기 : pivot_wider 활용하기

pivot_wider는 pivot_longer의 반대이다. 관측값이 여러 행에 걸쳐있을때 이를 열로 변환하는 방식이다. pivot_wider는 여기서는 두개의 주요 파라미터를 이해하면 된다. 
...
```{r, collapse=TRUE, eval=FALSE}
company_3 <- read.delim("HCCP_Head_3th.txt", header = TRUE)
company_4 <- read.delim("HCCP_Head_4th.txt", header = TRUE)

worker_3 <- read.delim("HCCP_Work_3th.txt", header = TRUE)
worker_4 <- read.delim("HCCP_Work_4th.txt", header = TRUE)
```

위 코드를 그대로 실행하면 `HCCP_Head_4th.txt`를 불러오는 과정에서 오류가 발생한다. 이 파일만 UTF-16 little-endian 방식으로 저장되어 있기 때문이다. 인코딩(encoding)은 한글이나 알파벳 같은 문자를 컴퓨터가 처리할 수 있도록 변환하는 규칙이다. 한글이 포함된 외부 자료를 불러올 때 인코딩 차이로 오류가 자주 발생하므로, 파일 형식을 확인한 뒤 `fileEncoding`을 지정한다. 이 책의 PDF에는 학습에 불필요한 원시 오류 메시지가 나타나지 않도록 위 실패 예제를 실행하지 않는다.

```{r,collapse=TRUE}
company_3 <- read.delim("HCCP_Head_3th.txt", header = TRUE)
company_4 <- read.delim(
  "HCCP_Head_4th.txt",
  header = TRUE,
  fileEncoding = "UTF-16LE"
)
worker_3 <- read.delim("HCCP_Work_3rd.txt", header = TRUE)
worker_4 <- read.delim("HCCP_Work_4th.txt", header = TRUE)
```
...
worker_3 %>% 
  count(W3_id3)
```
위 코드를 실행하면 `W3_id1`을 찾을 수 없다는 오류가 발생한다. Environment 창에서 변수명을 다시 살펴보면 첫 번째 변수는 `w3_id1`, 두 번째 변수는 `W3_id2`처럼 대문자와 소문자가 일관되지 않다. 이런 사소한 문제가 실제 데이터 전처리를 어렵게 만든다. 변수명을 하나씩 수정할 수도 있지만, 여기서는 네 데이터 프레임의 모든 변수명을 소문자로 통일한다. `dplyr`의 `rename_with(tolower)`를 사용하면 일괄 변환할 수 있다.

```{r, collapse=TRUE}
worker_3 %>% 
  rename_with(tolower)->worker_3
worker_4 %>% 
  rename_with(tolower)->worker_4
company_3 %>% 
  rename_with(tolower)->company_3
company_4 %>% 
  rename_with(tolower)->company_4
```
...
worker_3 %>% 
  count(w3_id2)
# 팀 ID 중 연구개발 직무(100번대)
worker_3 %>% 
  count(w3_id3) %>% 
  filter(w3_id3 >= 100 & w3_id3 < 200)
# 개인 ID: 앞의 10개 값만 확인
worker_3 %>% 
  count(w3_id4) %>% 
  slice_head(n = 10)
...
* background 변수 : w3_ind1(산업분류), w3_team(팀분류), w3_posit(팀장여부), w301_01, w301_02(현직장입사시기년, 월), w303_02(현재직급), w3_sex(성별), w3_birthy(출생년도), w3_marr(혼인상태), w3_edu
...
worker_3 %>% 
  select(
    w3_id1, w3_id3, w3_id4,
    w3_ind1, w3_team, w3_posit,
    w301_01, w301_02, w303_02,
    w3_sex, w3_birthy, w3_marr, w3_edu,
    w306_01, w306_02, w306_03, w306_04,
    w306_05, w306_06, w306_07, w306_08,
    w311_01, w311_02, w311_05, w311_06,
    w312,
    w329_01, w329_02, w329_03, w329_04,
    w333, w335,
    w336_01, w336_02,
    w337_01, w337_02
  ) -> worker_3_se
...
worker_4 %>% 
  select(
    w4_id1, w4_id3, w4_id4,
    w4_ind1, w4_team, w4_posit,
    w401_01, w401_02, w403_02,
    w4_sex, w4_birthy, w4_marr, w4_edu,
    w406_01, w406_02, w406_03, w406_04,
    w406_05, w406_06, w406_07, w406_08,
    w411_01, w411_02, w411_05, w411_06,
    w412,
    w429_01, w429_02, w429_03, w429_04,
    w433, w435,
    w436_01, w436_02,
    w437_01, w437_02
  ) -> worker_4_se
...
company_3 %>% 
  select(
    c3_id1, c3_ind1, c3_ksic1, c3_scale,
    c3a02_07,
    c3b02_01_01, c3b02_01_02, c3b02_01_03, c3b02_01_04,
    c3b02_02_01, c3b02_02_02, c3b02_02_05,
    c3b02_03_02, c3b02_03_03, c3b02_03_04,
    c3b02_05_01, c3b02_05_02,
    c3c01_01, c3c01_01_01, c3c01_02, c3c01_03,
    c3c01_06_01, c3c01_06_02,
    c3c02_03_01, c3c02_03_02, c3c02_03_03,
    c3c02_03_04, c3c02_03_05,
    c3c04_02_01, c3c04_02_02, c3c04_02_03
  ) -> company_3_se

company_4 %>% 
  select(
    c4_id1, c4_ind1, c4_ksic1, c4_scale,
    c4a02_07,
    c4b02_01_01, c4b02_01_02, c4b02_01_03, c4b02_01_04,
    c4b02_02_01, c4b02_02_02, c4b02_02_05,
    c4b02_03_02, c4b02_03_03, c4b02_03_04,
    c4b02_05_01, c4b02_05_02,
    c4c01_01, c4c01_01_01, c4c01_02, c4c01_03,
    c4c01_06_01, c4c01_06_02,
    c4c02_03_01, c4c02_03_02, c4c02_03_03,
    c4c02_03_04, c4c02_03_05,
    a_3rd
  ) -> company_4_se
```
...
```{r, collapse=TRUE}
#데이터 결합
# key 변수 이름 통일 및 연도별 flag 생성
company_3_se <- company_3_se %>%
  rename(company_id = c3_id1) %>%
  mutate(in_3rd = TRUE)
company_4_se <- company_4_se %>%
  rename(company_id = c4_id1) %>%
  mutate(in_4th = TRUE)
# flag 확인
company_3_se %>%
  count(in_3rd)
company_4_se %>%
  count(in_4th)
# full join
company_full <- company_3_se %>%
  full_join(company_4_se, by = "company_id") %>%
  mutate(
    in_3rd = replace_na(in_3rd, FALSE),
    in_4th = replace_na(in_4th, FALSE)
  )
# 연도별 기업 현황 확인
company_full %>%
  count(in_3rd, in_4th)
...
inner join 작업도 수행해보자. inner_join은 두 데이터에 공통으로 존재하는 key만 남긴다. 따라서 3차년도와 4차년도에 모두 존재하는 기업만 남게 된다. nrow()로 관측치 수를 확인하면 410개의 기업이 남아있는 것을 확인할 수 있다

```{r, collapse=TRUE}
#inner join
company_inner <- company_3_se %>%
  inner_join(company_4_se, by = "company_id")
#공통기업 수 확인
nrow(company_inner)
```

이밖에도 한쪽에만 존재하는 기업만 데이터에 포함시키는 anti_join()도 있다. 

```{r, collapse=TRUE}
# 3차년도 데이터에만 있는 관측치만 포함
company_3_only <- company_3_se %>%
  anti_join(company_4_se, by = "company_id")
nrow(company_3_only)
# 4차년도 데이터에만 있는 관측치만 포함
company_4_only <- company_4_se %>%
  anti_join(company_3_se, by = "company_id")
nrow(company_4_only)
```

### 근로자-기업 데이터 결합(many-to-one match)

마지막으로 수행할 데이터 결합은 근로자와 기업을 연결하는 것이다. HCCP데이터의 구조를 떠올려보면 기업 1개당 다수의 근로자 데이터가 존재한다. 두개의 데이터를 결합하려면 어떠한 key변수가 사용될까? 당연하게도 근로자 데이터의 기업 ID인 w3_id1과 기업 데이터의 기업 ID인 c3_id1일 것이다. 이를 데이터 구조로 생각해보면 데이터 X(근로자 데이터)의 여러 행이 데이터 Y(기업데이터)의 한 행과 결합된다는 것을 의미한다. 근로자 데이터를 기준으로 보면 이러한 결합을 many-to-one match라고 한다. 이러한 방식 역시 join 함수를 사용하면 된다. 앞 절에서 기업데이터의 key 변수(c3_id1, c4_id1)를 company_id로 통일하였기 때문에 c3_id1대신 company_id를 key 변수로 설정하여 조인해보자.

다만, one-to-one match 방식에 비해 결합된 결과물을 잘 살펴볼 필요가 있다.  group_by()와 summarise(count=n()) 명령어를 활용하여 주요 변수의 빈도를 확인해본 결과 결합이 잘 되어 있는 것을 확인할 수 있다. 

```{r, collapse=TRUE}
# 근로자-기업 데이터 결합
worker_company <- worker_3_se %>%
  left_join(company_3_se, by = c("w3_id1" = "company_id"))
# 결합 전후 관측치 수 확인
nrow(worker_3_se)
nrow(worker_company)
# 결합 결과 일부 확인
